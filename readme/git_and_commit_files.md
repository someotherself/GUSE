...
These will have a detached HEAD to the commit of that snap folder.
...
```text
An empty repository will still have an empty, initialized repo and a regular fetch can still be done. But by default, a git fetch will not include PRs. After a manual fetch, a session restart will be needed to update the folder structure.
